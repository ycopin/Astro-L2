% Time-stamp: <2017-01-10 18:04:58 ycopin>
...
\begin{Exercise}[Relation de Tully-Fisher]

  Les observations des galaxies spirales --- caractérisées par leur masse $M$,
  leur luminosité totale $L$ et leur rayon caractéristique $R$ --- montrent que
  leur courbe de rotation (vitesse de rotation en fonction de la distance) tend
  à grande distance vers une valeur limite $V_{\max}$.  L'objectif de cet
  exercice est de comprendre l'origine de la relation de Tully-Fisher (TF), qui
  exprime la magnitude absolue $\mathcal{M}$ des galaxies spirales en fonction
  de leur vitesse de rotation $V_{\max}$ :
  \begin{equation}
    \label{eq:1}
    \mathcal{M} = -10 \log V_{\max} + \mathrm{cte}
  \end{equation}

  \begin{enumerate}
  \item En utilisant le principe fondamental de la dynamique (ou toute
    expression qui en découle), relier la masse $M$ au rayon $R$ et à la
    vitesse de rotation $V_{\max}$.
  \item En supposant que le rapport masse-luminosité $M/L = \Upsilon$ est le
    même pour toutes les galaxies spirales et qu’elles ont toutes la même
    brillance de surface $\sigma = L/(\pi R^{2})$, établir que
    \begin{equation}
      \label{eq:9}
      L \propto V_{\max}^{4}
    \end{equation}
  ...
  \item En rappelant la relation entre la luminosité $L$ et la magnitude
    absolue $\mathcal{M}$, en déduire la relation de Tully-Fisher.
  \end{enumerate}
  Pour des galaxies de type~Sa, on observe que la relation de TF s'exprime par:
  \begin{equation}
    \label{eq:10}
    \mathcal{M}_{B} = -9,95 \log(V_{\max}\;[\si{km.s^{-1}}]) + 3,15.
  \end{equation}
  ...
  \begin{enumerate}[resume]
  \item Calculer la magnitude absolue $\mathcal{M}_{B}$ prédite par la relation
    de TF pour cette galaxie. En déduire, en rappelant l'expression du module
    de distance, la distance de cette galaxie.
  \item Sachant que $\mathcal{M}_{\odot B} = 5,47$, déterminer la luminosité
    $L_{B}$ (exprimée en $\Lsun_{B}$) de cette galaxie.
  \item Calculer la masse contenue dans un rayon $R = \SI{30}{kpc}$. En déduire
    le rapport $M/L_{B}$ (exprimé en $\Msun/\Lsun_{B}$).
  \end{enumerate}
\end{Exercise}

\begin{Answer}
  \begin{enumerate}
  \item On a classiquement $M = V_{\max}^{2}R/G$.
  \item On suppose que le rapport masse-luminosité $M/L = \Upsilon$ est le même
    pour toutes les galaxies spirales, ainsi :
    \begin{equation}
      \label{eq:3}
      L = M / \Upsilon = \frac{V_{\max}^{2}R}{\Upsilon G}
      \quad\text{d'où}\quad
    ...
    \end{equation}
  \item La magnitude absolue est donnée par
    $\mathcal{M} - \mathcal{M}_{\odot} = -2,5\log(L/\Lsun)$, ce qui permet
    d’obtenir la relation de Tully-Fisher :
    \begin{equation}
      \label{eq:5}
      \mathcal{M} = \mathcal{M}_{\odot} - 2,5\log \left(
        \frac{V_{\max}^{4}}{\sigma \pi \Upsilon^{2} G^{2} \Lsun}
      \right) = -10\log V_{\max} + \mathrm{cte}.
    \end{equation}
  \item $\mathcal{M}_{B} = -9,95 \log(324) + 3,15 = -21.83$. Le module de distance est
    donné par $B - \mathcal{M}_{B} = 5\log(D/\SI{10}{pc})$, et donc sa
    distance $D = 10^{(B - \mathcal{M}_{B})/5}\times\SI{10}{pc} = \SI{46,3}{Mpc}$.
  \item $\mathcal{M}_{B} - \mathcal{M}_{\odot B} = -2,5\log(L_{B}/\Lsun_{B})$
    donc $L_{B}/\Lsun_{B} = 10^{-0,4(\mathcal{M}_{B} - \mathcal{M}_{\odot B})} =
    8,3\,10^{10}$.
  \item La masse contenue dans un rayon $R = \SI{30}{kpc}$ se détermine par
    $M = V_{\max}^{2}R/G = \SI{7.3e11}{\Msun}$ avec
    $G = \SI{4.302e-6}{km^{2}.s^{-2}.\Msun^{-2}}$. Finalement,
    pour NGC~2639, $M/L_{B} = 8,8 \Msun/\Lsun_{B}$.
  \end{enumerate}
\end{Answer}
...
\begin{Answer}
  ...
  \item
    $$
    H \defeq \frac{\dot{R}}{R} = \frac{\gamma}{t} =
