% Time-stamp: <2017-01-10 19:13:23 ycopin>
...
\DeclareSIUnit\Rearth{\ensuremath{\mathsf{\emph{R}}_{\oplus}}}
\DeclareSIUnit\Mearth{\ensuremath{\mathsf{\emph{M}}_{\oplus}}}
...
\begin{Exercise}[Équilibre radiatif de la Terre]
  ...
  Soit $\Lsun$ la puissance lumineuse du Soleil, $R_{\oplus}$ le rayon de la
  Terre, $D$ la distance Terre-Soleil et $T_{\oplus}$ la température effective
  à la surface de la Terre :
  \begin{enumerate}
  \item Écrire la relation entre $\Lsun$ et $f(D)$, le flux lumineux à une
  ...
  \item Écrire l'égalité des puissance reçue et rayonnée à la surface
    de la Terre, en déduire l'expression de la température $T_{\oplus}$ en
    fonction des données du problème.
  \item Application numérique: $\Lsun = \SI{3,85e26}{W}$,
    $\Rearth = \SI{6400}{km}$, $D = \SI{1,500e11}{m}$,
...
\begin{Answer}
  \begin{enumerate}
  \item $f(D) = \Lsun / 4\pi D^2$, $P_{r} = \Lsun R^2/(4 D^2)$
  \item $f_e = \sigma T_{\oplus}^4$ et $P_e = 4\pi R^2 \sigma T_{\oplus}^4$
  \item $P_e = P_r$ d'où $T_{\oplus} = \left( \Lsun / (16\pi D^2\sigma)
    \right)^{1/4} = 278$~K.
  \end{enumerate}
\end{Answer}
...
\begin{Exercise}[Interférométrie]
  ...
  de l'exercice~\ref{exo:parallaxe}, mesuré par interférométrie.
  Calculer leur rayon $R$ (on rappelle les distances déterminées dans
  l'exercice précédent) et, à l'aide de ce résultat, attribuer à
  chaque étoile sa classe de luminosité parmi les suivantes : I,
  III,~V.
  \begin{center}
    \begin{tabular}{lccc}
      \toprule
...
\begin{Exercise}[Invariant de Runge-Lenz (facultatif)]
  \color{Sepia}
...
\begin{Exercise}[Densité des galaxies dans l'Univers (examen 2013)]
...
\begin{Answer}
  \begin{enumerate}
  \item $\ell = h/\sin b$ et $A = \ell/h \times A_{0} = A_{0}/\sin b$.
...
\begin{Exercise}[Propriétés «~physiques~» de la classification]
  En vous servant du tableau~\ref{tbl:hubble}, répondez aux questions
  suivantes :
  \begin{enumerate}
...
\begin{Exercise}[Le centre galactique]
  La Fig.~\ref{fig:S2} montre l'orbite de l'étoile ayant la plus
...
\begin{SCfigure}[0.5]%[htp]
  ...
  \caption{Orbite de l'étoile S2 autour du centre galactique SgrA*
    (Schödel et al.~2002, \emph{A star in a 15.2-year orbit around the
      supermassive black hole at the centre of the Milky Way}, Nature,
    \textbf{419}, 694).}
  \label{fig:S2}
\end{SCfigure}
...
\begin{Answer}
  ...
  a~[\u{UA}] = a~[''] \times R~[\u{pc}] = 1,011\e{3}
  $$
...
\begin{Exercise}[Recensement]
  Dans la Table~\ref{tbl:listgalac}, compter le nombre de galaxies
  ayant un diamètre plus petit que 6~kpc, et celles ayant un diamètre
  plus grand. Quelles sont les galaxies qui dominent en nombre ? Et en
  luminosité totale ?
\end{Exercise}
...
% Référence: Pr. G. Meylan, EPFL, printemps 2010
\begin{Exercise}[Relation de Tully-Fisher (examen 2015)]
  
  Les observations des galaxies spirales --- caractérisées par leur masse
  $\mathcal{M}$, leur luminosité totale $L$ et leur rayon caractéristique $R$
  --- montrent que leur courbe de rotation (vitesse de rotation en fonction de
  la distance) tend à grande distance vers une valeur limite $V_{\max}$.
  L'objectif de cet exercice est de comprendre l'origine de la relation de
  Tully-Fisher (1977, \emph{A new method of determining distances to galaxies},
  A\&A, \textbf{54}, 661), qui exprime la magnitude absolue $M$ des galaxies
  spirales en fonction de leur vitesse de rotation $V_{\max}$ :
  $$
  M = -10 \log V_{\max} + \mathrm{cte}
  $$

  \begin{enumerate}
  \item En utilisant le principe fondamental de la dynamique (ou toute
    expression qui en découle), relier la masse $\mathcal{M}$ au rayon $R$ et à
    la vitesse de rotation $V_{\max}$.
  \item En supposant que le rapport masse-luminosité $\mathcal{M}/L = \Upsilon$
    est le même pour toutes les galaxies spirales et qu’elles ont toutes la
    même brillance de surface $\sigma = L/(\pi R^{2})$, établir que
    $$
    L \propto V_{\max}^{4}
    $$
    où l'on précisera la constante de proportionnalité.
  \item En rappelant la relation entre la luminosité $L$ et la magnitude
    absolue $M$, en déduire la relation de Tully-Fisher (TF).
  \end{enumerate}
  Pour des galaxies de type~Sa, on observe que la relation de TF s'exprime par:
  \begin{equation}
    \label{eq:10}
    M_{B} = -9,95 \log(V_{\max}\;[\si{km.s^{-1}}]) + 3,15.
  \end{equation}
  La galaxie NGC~2639, de type~Sa, possède une vitesse de rotation
  $V_{\max} = \SI{324}{km.s^{-1}}$ (atteinte pour $R = \SI{30}{kpc}$) et une
  magnitude apparente $B = 11,5$~mag.
  \begin{enumerate}[resume]
  \item Calculer la magnitude absolue $M_{B}$ prédite par la relation de TF
    pour cette galaxie.  En déduire, en rappelant l'expression du module de
    distance, la distance de cette galaxie.
  \item Sachant que $\Msun_{B} = 5,47$, déterminer la luminosité $L_{B}$
    (exprimée en $\Lsun_{B}$) de cette galaxie.
  \item Calculer la masse contenue dans un rayon $R = \SI{30}{kpc}$. En déduire
    le rapport $\Upsilon_{B} = \mathcal{M}/L_{B}$ (exprimé en $\Msun/\Lsun_{B}$).
  \end{enumerate}
\end{Exercise}
...
\begin{Exercise}[Fonction de luminosité]
  La fonction de luminosité $\Phi(L)$ des galaxies s'exprime généralement à
  l'aide de la fonction de Schechter (1976) :
  $$
  \Phi(L)\,\d L =
  ...
  \begin{SCfigure}%[htp]
    ...
    \caption{Fonction de luminosité des galaxies dans les amas d'Abell
      (Schechter, 1976, \emph{An analytic expression for
    the luminosity function for galaxies}, ApJ, \textbf{203}, 297).} 
    \label{fig:schechter76}
  \end{SCfigure}
\end{Exercise}
...
\begin{Exercise}[Fréquence de collision dans un amas]
  ...
  \item les galaxies sont supposées animées d'une vitesse
    \emph{relative} quadratique moyenne $v_{g}$ (c.-à-d. que la
    vitesse quadratique moyenne est de $v_{g}/\sqrt{2}$).
  \end{itemize}
...
\begin{Exercise}[Autres applications du théorème du Viriel]
  ...
  tableau ci-dessous, donnant les rayons, masses et vitesses
...
\begin{Answer}
  ...
  Fig.~\ref{fig:grav}, la cabine de gauche, supposée accélérée vers le
  ...
  \begin{figure}
    ...
    \begin{subfigure}[b]{0.4\textwidth}
      \centering
      \includegraphics[width=\textwidth]{grav}
      \caption{}
      \label{fig:grav}
    \end{subfigure}
    \begin{subfigure}[b]{0.4\textwidth}
      \centering
      \includegraphics[width=\textwidth]{351922_281}
      \caption{}
      \label{fig:H0}
    \end{subfigure}
    \caption{}
...
\begin{Exercise}[... la même partout ?]
  La radiogalaxie 3C~171\footnote{La 171e entrée dans le troisième catalogue
...
\begin{Answer}
  ...
  est universel : $X_0=H_0$ (Fig.~\ref{fig:H0}).
\end{Answer}
...
\begin{Exercise}[Le facteur d'expansion de l'espace]
  ...
  par seconde et par mégaparsec ($\u{km~s^{-1}~Mpc^{-1}}$).  Quelle
  est la valeur de l'accroissement annuel d'un kilomètre ?  Que vaut
  $H_{0}$ en $\u{kpc~Gan^{-1}~Mpc^{-1}}$ ?
\end{Exercise}

\begin{Answer}
  $1\;\u{Mpc} = 3,09\e{19}\;\u{km}$ et $1\;\u{an} = 3,16\e7\;\u{s}$,
  donc $70\;\u{km~s^{-1}~Mpc^{-1}} = 2,27\e{-18}\;\u{km~s^{-1}~km^{-1}}
  = 71,6\;\u{nm~an^{-1}~km^{-1}}$.
\end{Answer}
...
\begin{Answer}
  Puisque $[G] = L^{3}\,M^{-1}\,T^{-2}$, on a $[\rho_{c}] =
  M\,L^{-3}$: c'est une masse volumique.  $\rho_{C} =
  9,20\e{-27}~\u{kg\;m^{-3}} = 5,54\;\text{nucléons}\;\u{m^{-3}} =
  1,36\e{11}\;\Msun\;\u{M\pc}^{-3}$, soit environ une galaxie
  (massive) par \u{M\pc^{3}}, densité caractéristique de l'Univers
  local.
\end{Answer}

\begin{Exercise}[Âge de l'Univers]
  À partir de la définition de la «~constante~» de Hubble $H$ et de la
  relation $R \propto t^{2/3}$ entre le facteur d'échelle $R$ et le
  temps $t$ dans le cas d'un Univers euclidien ($k=0$) dominé par la
  matière ($\Omega_{M}=1$, $\Omega_{\Lambda}=0$), trouver la relation
  entre la valeur actuelle $H_{0}$ de la constante et l'âge de
  l'Univers $t_{0}$.
\end{Exercise}
...
\begin{Exercise}[Modèle FLRW (examen 2011)]
...
\begin{Answer}
  ...
  \item $\dot{\rho}/\rho = -3(1+w) \dot{R}/R_{0}$ soit $\frac{\d}{\d
      t}\ln(\rho/\rho_{0}) = -3(1+w)\frac{\d}{\d t}\ln(R/R_{0})$. On
    vérifie que $\rho/\rho_{0} = (R/R_{0})^{-3(1+w)}$ est bien
  ...
  \item En égalisant les expressions de $\dot{R}$, on obtient
    $\gamma = \dfrac{1}{1+\eta} = \dfrac{2}{3(1+w)}$.

  \item $H \defeq \dot{R}/R = \gamma/t = \dfrac{2}{3(1+w)}\dfrac{1}{t}$
...
\begin{Answer}
  ...
  \item
    ...
    \item[Castor (Gem, $\delta = +31\deg56'$) circumpolaire:]
      ...
      (Fig \ref{fig:castor}) : $\phi>=90\deg-\delta$
      \begin{figure}[htp]
        \centering
        \begin{subfigure}[b]{0.3\textwidth}
          \centering
