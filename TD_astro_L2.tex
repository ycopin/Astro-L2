% Time-stamp: <2012-03-07 15:32 ycopin@lyopc469>
...
\begin{Answer}
  ...
  \item Son rayon vaut donc $R = \theta_* d / 2 = 3,8 \times
    10^{-8}\;\u{pc} = 1,2 \times 10^9\;\u{m} = 1,7R_{\odot}$.
  \end{itemize}
...
\begin{Answer}
  ...
  \item Le demi-grand axe de l'orbite relative vaut donc $a = \theta d
    = 1,45 \times 10^{-4}\;\u{pc} = 4,49 \times 10^{12}\;\u{m} = 30$~\UA.
  \item La $3^{\mathrm{ème}}$ loi de Kepler donne la somme des masses
    : $M1 + M2 = 5,97 \times 10^{31}\;\u{kg} = 30 M_{\odot}$ .
...
\begin{Exercise}[Système binaire d'Algol]
...
\begin{Answer}
  ...
  \item Le demi-grand axe de l'orbite relative vaut donc $a = \theta d
    = 3,17 \times 10^{-7}~\u{pc} = 9,77 \times 10^9~\u{m} = 0,065~\UA =
    14R_{\odot}$.
  \item La 3ème loi de Kepler donne la somme des masses :
    $M_1+M_2 = 4\pi^2 a^3/GT^2 = 8,98 \times 10^{30}~\u{kg} =
    4,52M_{\odot}$.
...
\begin{Answer}
  \begin{align*}
    ...
    L &= 2000[\u{pc}] .(3\times 10^{18} [\u{cm/pc}]) = 6\times 10^{21} [\u{cm}] \\
  ...
  $$
  n_g = \frac{1.75\times  10^{9}}{6\times  10^{21}} = 2.9\times 
  10^{-13}~[\u{grain/cm^{3}}]
  $$
  Masse volumique des grains dans le MIS :
  $$
  1 [\u{g/cm^{3}}]\times \frac{4a^{3}}{3}~[\u{cm^3/grain}] 2.9\times  10^{-13}
  [\u{grain/cm^3}]= 3.87\times  10^{-28}~[\u{g/cm^3}]
  $$
  ...
  \frac{\text{Masse volumique des grains}}{\text{Masse volumique du
      gaz}} = \frac{3.87\times 
    10^{-28}~[\u{g/cm^3}]}{1.34\times  10^{-23}~[\u{g/cm^3}]} = 2.9\times 10^{-5}
  $$
...
\begin{Answer}
  ...
  \item Pour la partie du spectre aux grandes longueurs d'onde, le
    terme $\exp(a'/\lambda)$ tend progressivement vers 1, donc $W'$
    devient égal à $W$. Les spectres rougis et non rougis sont
    identiques.
  \end{enumerate}
\end{Answer}
...
\begin{Answer}
  \begin{enumerate}
  \item $a=V^2/R$
  \item $mV^2/R = GmM(R)/R^2$, soit $V^2 = GM(R)/R$. $V$ doit
    décroitre avec $R$ avec la puissance $-1/2$ donc tendre doucement
    vers zéro.
  \item Si on arrive à une valeur limite $V_0$, la repartition de
    masse dans la galaxie est peut-être à revoir, introduction de la
    notion de matière noire qui accroît la masse mais est invisible à
    nos pauvres « yeux ».
  \item 
    \begin{equation*}
...
\begin{Exercise}[Le centre galactique]
  ...
  gravitationnelle, vaut $6,6742.10^{-11}$~\u{m^3 kg^{-1} sec^{-2}}.
  La masse visible étant estimée à environ 106 masses solaires, en
...
\begin{Exercise}[Calcul de la fréquence des collisions dans un amas]
  ...
  suivantes :
  \begin{itemize}
  \item il est supposé sphérique, de diamètre $D_{\mathrm{a}}$ en pc,
  \item il contient $N_{\mathrm{g}}$ galaxies identiques et réparties
  ...
  \item Donner l'expression de la densité numérique de galaxies (le
    nombre de galaxies par unité de volume) $n_{\mathrm{g}}$ dans l'amas.

  \item On suppose que les galaxies ont un diamètre typique
    $D_{\mathrm{g}}$. En modélisant les galaxies comme
    des sphères dures (boules de billard) de diamètre
    $D_{\mathrm{g}}$, donner l'expression de la section
    efficace $S_{\mathrm{eff}}$ lors d'une
    collision/interaction entre deux galaxies.

  \item Donner l'expression du nombre de collisions/interactions que
    subit une galaxie de l'amas pendant le temps $\Delta t$ (en
...
\begin{Exercise}[Théorème du Viriel scalaire]
  ...
  \item On considère un système d'étoiles binaires en équilibre. Pour
    simplifier les calculs, les étoiles sont prises de masses égales à
    $M_\odot$ et leurs vitesses sont considérées comme égales ; on
    pose $v=v_1=v_2$.

    Expliciter la relation entre la distance entre ces deux étoiles et
    leur vitesse et calculer cette vitesse en~km/s pour les distances
...
\begin{Answer}
  ...
  \item Théorème du Viriel : $2K + U = 0$, soit :
    \begin{align*}
      U &= -2K \\
      &= -\frac{GM_\odot M_\oplus}{D} \\
      D &= -\frac{GM_\odot M_\oplus}{U} \\
      &= \frac{GM_\odot}{v_\oplus^2}
    \end{align*}
    avec $v_\oplus = 30$~km/s, on trouve bien $D=1$~UA.
  \item Le même genre de calculs amène à :
    $$
    v = \sqrt{\frac{G M_\odot}{2 D}}
    $$
    on trouve :
    \begin{itemize}
    \item pour $D=0.5$~UA, $v= 470$~km/s
