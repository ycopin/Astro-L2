% Time-stamp: <2013-06-13 16:44:05 ycopin>
...
% D'après http://cc.oulu.fi/~jpoutane/teaching/ism07.html
  ...
  réalité de la distribution des galaxies dans l'Univers, mais résulte
  ...
  \item Dans ces conditions, montrer que le nombre cumulé $N(m,b)$ de
    ...
    $$
    \log N (m,b) = 0,6\,m - 0,6A_{0}\frac{1}{\sin b} + K.
    $$
...
\begin{Answer}
  ...
  \item $N(m,b) = \rho \times 4\pi d^{3}/3$ avec $\log(d/\mathrm{pc})
    = 1+0,2(m-M-A)$ donc $\log N(m,b) = 0,6m - 0,6A - 0,6 M +
    \log(\rho/\mathrm{pc}^{3}) + \log(4\pi/3) + 3$.
  \item $\log(\rho/\mathrm{pc}^{3}) = -16,52$ soit $\rho =
    30~\mathrm{Mpc}^{-3}$.
  \end{enumerate}
\end{Answer}
...
\begin{Answer}
  ...
  $$
  \frac{(a/a_{\oplus})^{3}}{(T/T_{\oplus})^{2}} =
  M_{\mathrm{SgrA}*}/M_{\odot} \qquad\text{avec}\qquad
