% Time-stamp: <2011-09-08 16:25:32 ycopin>
...
\begin{Answer}
  ...
  10^{-8}~Wm^{-2}K^{-4}$, on trouve la température effective du Soleil
  $T_\odot \approx 5770~K$
\end{Answer}
...
\begin{Answer}
  ...
  $$
  d = \frac{1}{p} = \frac{1}{50 \times 10^{-6}} = 20~\mathrm{kpc}
  \quad\text{avec FAME}
  $$
  et
  $$
  d = \frac{1}{p} = \frac{1}{4 \times 10^{-6}} = 250~\mathrm{kpc}
  \quad\text{avec GAIA}
  $$
\end{Answer}
...
\begin{Answer}
  ...
  \begin{itemize}
  \item $\mu = 47,3 \pm 0,8 mas.an^-1 = (2,29 \pm 0,04) \times 10^{-7}
    rad.an^-1$
  \item $v_r = 10,1 \pm 0,3 km.s^-1 = (1,02 \pm 0,03) \times 10^{-5}
    pc.an^-1$
...
\begin{Exercise}[Types spectraux]
  ...
  Rappel de la loi de Wien: $\lambda_{\max} T = 2898$~$\mu$m.K.
\end{Exercise}
...
\begin{Answer}
  ...
  \begin{center}
    \begin{tabular}{|c|c|c|c|c|c|c|}
      ...
      \hline
      $M_{totale}$ ($10^{10}M_{\odot}$) & & 22.6 & 32.4 & 19.0 & 7.9 &
      1.6 \\
      \hline
      $M_{gaz}$ ($H$ neutre en $10^{9}M_{\odot}$) & 1.24 & 5.62 &
      15.14 & 15.85 & 9.33 & 2.40\\
      \hline
      $M_{gaz}/M_{totale}$ & & 0.03 & 0.05 & 0.08 & 0.11 & 0.15 \\
      \hline
...
\begin{Exercise}[La matière noire]
  ...
  du centre de la galaxie.  Écrire la loi de Newton pour cette étoile
  : l'accélération $a$ en fonction de la masse $M$ incluse dans le
  rayon $R$.  En déduire la loi de décroissance de $V$ en fonction du
  rayon $R$ de l'étoile, en supposant que la masse $M$ reste constante
...
\begin{Exercise}[Morphologie de notre galaxie]
  ...
  pas symétrique.
  \begin{enumerate}
  \item Examiner cette image et essayer de mesurer la hauteur du bulbe
    ; à droite, puis à gauche du centre de la Galaxie
...
On écrit donc:
$$
P_{Marcel} = G \frac{M_{Marcel}M_{\oplus}}{R^2}
$$

Soit encore:
$$
M_{Marcel} = \frac{P_{Marcel}R^2}{G M_{\oplus}}
$$

et donc
$$
M_{Marcel} = \frac{ 6370000^2 \times 700 }{ 6,672 \times 10^{-11}
  \times 5,976 \times 10^{24} } = 71,23~kg
$$

Cette valeur s'obtiendrait aussi en utilisant la formule $P =
M_{Marcel}\times g$, où $g$ est l'accélération de la pesanteur à
Lyon. Mais l'énoncé ne précise pas la valeur de $g$ à Lyon...

Pour Naomi, on peut utiliser le fait que $M_{Naomi}/M_{Marcel} =
P_{Naomi}/P_{Marcel}$, puisque les deux personnes sont soumises au
même champ de gravité. Et donc:
$$
M_{Naomi} = M_{Marcel} \frac{P_{Naomi}}{P_{Marcel}} = 71,23 \times
\frac{58}{70} = 59,02~kg
$$

D'où $F_{Naomi-Marcel} = ( 6,672 10^{-11} \times 71,23 \times 59,02 )
/ 1 = 2,8 10^{-7}~N$.

Et la masse qui présenterait un poids égal à cette valeur en cet
endroit serait de : $M = ( 63700002 \times 2,8 10^{-7} ) / ( 6,672
10^{-11} \times 5,976 10^{24} ) = 2,85 10^{-8} kg = 28,5~\mu g$.

Qui peut se calculer aussi, d'après une remarque déjà faite, par $M =
71,23 \times 2,8 10^{-7} / 700$. L'attraction gravitationnelle est une
force \emph{très faible}, c'est même la plus faible des quatre forces
...
\begin{Exercise}[Calcul du temps de Hubble]
  ...
  est $H_0 = 65\;\mathrm{km s^{-1} Mpc^{-1}}$ Quelle conclusion
  pouvez-vous en tirer sur l'âge réel de l'univers?
\end{Exercise}

\begin{Answer}
  Le temps de Hubble est défini par: $1/t_{H0} = H_0 =
  65~\mathrm{km~s^{-1}~Mpc^{-1}}$. Il reste simplement à convertir les
  Mpc (megaparsecs) en kilomètres, et il vient : $1/t_{H0} =
  65~\mathrm{km~s^{-1}} [3,26 10^{19}~\mathrm{km}]^{-1} = 20 \times
