% Time-stamp: <2011-09-09 11:39:35 ycopin>
...
\begin{Exercise}
  ...
  satellite: $\Delta p=50$~\u{\mu as} pour FAME et 4~\u{\mu as} pour
  GAIA.
\end{Exercise}
...
\begin{Answer}
  ...
  \begin{align*}
    \frac{\Delta d}{d} &= \frac{\Delta p}{p} = \frac{50 \times
      10^{-6}}{10^{-2}} = 5 \times 10^{-3} = 0.5 \% &\text{avec FAME} \\
    \frac{\Delta d}{d} &= \frac{\Delta p}{p} = \frac{4 \times
      10^{-6}}{10^{-2}} = 4 \times 10^{-4} = 0.04 \% &\text{avec GAIA} 
  \end{align*}
  ...
  \begin{align*}
    ...
    &\text{avec FAME} \\
    d &= \frac{1}{p} = \frac{1}{4 \times 10^{-6}}
    = 250~\u{kpc} &\text{avec GAIA}
  \end{align*}
\end{Answer}
...
\begin{Answer}
  ...
  \begin{center}
    \begin{tabular}{|c|c|c|}
      \hline
      $\lambda_m$ & $T_{\mathrm{eff}}(K)$ & Type spectral \\
      \hline
...
\begin{Exercise}[Application du théorème du Viriel]
  ...
  astronomiques pratiques, ainsi
  $G=0.004301$~\u{pc.km^{2}.s^{-2}.M_{\odot}^{-1}})
...
\begin{Exercise}[Temps cinématique]
  ...
  pratiques, ainsi $G=0.004301$~\u{pc.km^{2}.s^{-2}.M_{\odot}^{-1}})
  \begin{center}
    \begin{tabular}{|c|c|c|c|}
      \hline
...
\begin{Answer}
  ...
  Terre au Soleil ... La valeur de deux fois huit minutes est
  elle-même approximative. Roemer aurait plutôt trouvé $2 \times
  10^8$~m/s, dit-on. La première détermination sérieuse de l'Unité
  Astronomique est due à Cassini et Richer, en 1671. Il faudra
  attendre le XXe siècle et les méthodes d'écho radar pour disposer de
  mesures très précises de cette valeur.
\end{Answer}
...
\begin{Exercise}[Le facteur d'expansion de l'espace]
  ...
  pratique pour les astronomes, mais ne parle guère au sens commun.
  Passons donc dans des unités plus usuelles : si l'on suppose que
  $H_0 = 65$~\u{km s^{-1} Mpc^{-1}}, quelle est la valeur de
  l'accroissement annuel d'un kilomètre ?
\end{Exercise}
...
\begin{Exercise}[Calcul du temps de Hubble]
  ...
  est $H_0 = 65$~\u{km s^{-1} Mpc^{-1}} Quelle conclusion pouvez-vous
  en tirer sur l'âge réel de l'univers?
\end{Exercise}

\begin{Answer}
  Le temps de Hubble est défini par: $1/t_{H0} = H_0 =
  65$~\u{km~s^{-1}~Mpc^{-1}}. Il reste simplement à convertir les Mpc
  (megaparsecs) en kilomètres, et il vient : $1/t_{H0} =
  65~\u{km~s^{-1}} [3,26 10^{19}~\u{km}]^{-1} = 20 \times 10^{-19}$~s,
  d'où $t = 5 \times 10^{17}~s = 16 \times 10^9$~années. On peut donc
  penser que l'univers est âgé de moins de 16~milliards d'années.
\end{Answer}
